    ...
    <div style={{display:'flex',flexDirection:'column',gap:'18px'}}>
        ...
        {tabs.slice(1).map((seg,i)=>{
          ...
          return (
            <div key={seg} style={{...card(),borderTop:`3px solid ${SEG_COLOR[seg]}`,cursor:'pointer'}} onClick={()=>setFilter(seg)}>
              ...
              <p style={{margin:'0 0 2px',fontSize:'24px',fontWeight:'800',color:'#1a1d3a'}}>{count.toLocaleString('en-IN')}</p>
      ...
      <div style={card()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px',flexWrap:'wrap',gap:'12px'}}>
          <p style={{fontWeight:'700',fontSize:'15px',color:'#1a1d3a',margin:0}}>All Customers</p>
          <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
            <div style={{background:'#f4f6fd',borderRadius:'8px',display:'flex',alignItems:'center',gap:'6px',padding:'0 12px',height:'34px'}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search customer ID..." style={{background:'transparent',border:'none',outline:'none',fontSize:'13px',color:'#1a1d3a',width:'130px',fontFamily:'inherit'}}/>
            </div>
        ...
        {customers.length===0 ? (
          <div style={{textAlign:'center',padding:'40px',color:'#9fa8c7'}}>
            <p style={{fontWeight:'600',color:'#1a1d3a',marginBottom:'6px'}}>No customer data yet</p>
            <p style={{fontSize:'13px'}}>Upload a customer CSV file to see all customers here.</p>
          </div>
        ) : (
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:'13px'}}>
              <thead>
                <tr style={{borderBottom:'2px solid #f0f2ff'}}>
                  {['Customer ID','Days since last purchase','Total orders','Total spend','Group','Status'].map(h=>(
              ...
              <tbody>
                {filtered.slice(0,200).map((c,i)=>(
                  <tr key={i} style={{borderBottom:'1px solid #f8f9fc'}}>
                    ...
                    <td style={{padding:'11px 12px',color:'#6b7280'}}>{c.recency} days ago</td>
                    <td style={{padding:'11px 12px',color:'#6b7280'}}>{c.frequency} orders</td>
                    <td style={{padding:'11px 12px',color:'#1a1d3a',fontWeight:'600'}}>{INR(Math.round(c.monetary))}</td>
                    ...
                    <td style={{padding:'11px 12px'}}>
                      <span style={{padding:'3px 10px',borderRadius:'20px',fontSize:'11px',fontWeight:'600',
                        ...
                        {c.recency<90?'Active':c.recency<200?'Cooling off':'Inactive'}
                      </span>
                    </td>
                  </tr>
                ))}
            ...
            {filtered.length>200 && (
              <p style={{textAlign:'center',padding:'14px',color:'#9fa8c7',fontSize:'12px'}}>
                Showing first 200 of {filtered.length.toLocaleString('en-IN')} customers. Use filters to narrow down.
              </p>
            )}
          </div>
        )}
